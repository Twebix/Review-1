...
public class TypeChecker {
    ...
    private void checkStatement(ASTNode node) {
        ...
        if (node instanceof VarDeclNode)
            checkVarDecl((VarDeclNode) node);
        else if (node instanceof AssignNode)
            checkAssign((AssignNode) node);
        else if (node instanceof IfNode)
            checkIf((IfNode) node);
        else if (node instanceof WhileNode)
            checkWhile((WhileNode) node);
        else if (node instanceof PrintNode)
            checkExpr(((PrintNode) node).expression);
    }
    ...
    private String checkExpr(ASTNode node) {
        ...
        if (node instanceof BinaryOpNode) {
            ...
            if ("দশমিক".equals(lt) || "দশমিক".equals(rt))
                return "দশমিক";
            return "সংখ্যা";
        }
    ...
    private void popScope() {
        scope = scope.parent != null ? scope.parent : scope;
    }
